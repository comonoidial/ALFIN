...
c2aFix :: QName -> String -> [VarDef] -> ShapeType -> TopExp -> Context [Function]
...
c2aTopExp xs (PType pt) (SimpleExp (VarExp x)         ) = return ([], AsmBlock xs (NodeReturn (BoxCon $ defaultPrimBox pt) [Right $ pv x]))
...
c2aTopExp xs (PType pt) (SimpleExp (FunVal f ys)) = do
  ...
  case (snd fk) of
    ...
    _ -> error $ show fk
c2aTopExp xs RefType (SimpleExp (VarExp x)         ) = return ([], AsmBlock xs (TailCall (EvalRef (rv x)) [])                          )
...
c2aCallingExp (VarExp x) = return ([], EvalRef (rv x), [])
...
buildConNodeArg :: QName -> [(String,ArgKind)] -> Context (AsmTag, [Argument])
...
buildConNode :: QName -> [VarDef] -> Context (AsmTag, [Parameter])
